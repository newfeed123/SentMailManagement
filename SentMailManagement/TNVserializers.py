        ...
        try:
            ...
            if instance.active:
                ...
                for systemsenderconfig in all_systemcsenderconfigs:
                    if instance != systemsenderconfig and instance.active:
...
class SystemMailSenderConfigUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        try:
            ...
            all_systemsenderconfigs = SystemMailSenderConfig.objects.filter(is_default=True)
            for systemsenderconfig in all_systemsenderconfigs:
                if instance != systemsenderconfig:
                    systemsenderconfig.active = False
                    systemsenderconfig.save()                    
    ...
    def create(self, validated_data):
        try:
            ...
            if uuid_mail_target is not None:
                ...
                instance.send_mail_front_end_to_many_people(uuid_mail_target, instance.mail_sender["uuid"])
...
class SystemMailMailCreateSerializer_ComposeMail(serializers.ModelSerializer):
    ...
    list_mail_target = serializers.CharField()
    ...
    def create(self, validated_data):
        try:
            ...
            ########### Process (list_mail_target)Many2Many POST #############
            if "list_mail_target" in validated_data:
                list_mail_target = validated_data.pop('list_mail_target')
            if "list_mail_target" in request_data:
                list_mail_target = request_data.pop('list_mail_target') 
            ...
            if "list_mail_target" in locals():
                # instance.list_mail_target.clear()
                # clear value not exist in "list_mail_target":
                crr_list_mail_target_list = instance.list_mail_target
                if crr_list_mail_target_list:
                    instance.list_mail_target = []
                    for crr_list_mail_target in crr_list_mail_target_list:
                        if crr_list_mail_target["uuid"] not in list_mail_target:
                            instance.delete_list_mail_target_values(crr_list_mail_target["uuid"])
                        else:
                            # update if exist in "list_mail_target"
                            instance.update_list_mail_target_values(crr_list_mail_target["uuid"])
                
                for obj in list_mail_target:
                    if crr_list_mail_target_list and obj not in crr_list_mail_target_list:
                        instance.add_list_mail_target_values(obj)
                    else:
                        instance.add_list_mail_target_values(obj)
            ...
            instance.compose_mail_front_end_to_many_people(instance.list_mail_target, None, instance.content)
        ...
        try:
            ...
            if instance.is_default:
                ...
                for systemserverconfig in all_systemcserverconfigs:
                    if instance != systemserverconfig and instance.is_default:
