...
class SystemMailSentMailSentMailManagement {
                ...
                var j_ele_mail = $("#mailSystemMailSentMailSentMailManagementInputId");
                    if (j_ele_mail.length > 0){
                        ...
                        if(mail_select_value == undefined || mail_select_value == null){
                            ...
                            j_ele_mail.append(new Option(mail_name, mail_value,true,true));
                        }
                    }
            }
            catch(err) {
    ...
    tFillEditModalForm(formId = null) {
        self = this
            ...
            try{
                ...
                if (j_ele_content.length > 0){
                    if (j_ele_content.attr('name') != 'uuid'){
                        if (MPSSystemMailSentMailSentMailManagementEditSystemMailSentMail_ckeditor_list.hasOwnProperty('#contentSystemMailSentMailSentMailManagementEditModalInputId')){
                            MPSSystemMailSentMailSentMailManagementEditSystemMailSentMail_ckeditor_list["#contentSystemMailSentMailSentMailManagementEditModalInputId"].setData(self.content);
                        }
                    }
                }
                else{
            ...
            try{
                var j_ele_mail = $("#mailSystemMailSentMailSentMailManagementDetailInputId");
                    if (j_ele_mail.length > 0){
                        ...
                        var mail_value = self["mail"]["uuid"];
                        var mail_name = `<a href="${org_app_prefix}/SentMailManagement/SystemMailMail/detail/${mail_value}/" class="text-gray-600 text-hover-primary me-2">${self["mail"]["name"]}</a>`
    ...
    tFillDetailModalForm(formId = null) {
        self = this
            ...
            //     if (j_ele_content.length > 0){
            //         if (j_ele_content.attr('name') != 'uuid'){
            //             j_ele_content.html(self["mail"]["content"]).change();
            //         }
            //     }
            //     else{
            //         // j_ele_content.html(null);
            ...
            try{
                ...
                if (j_ele_content.length > 0){
                    if (j_ele_content.attr('name') != 'uuid'){
                        // j_ele_content.html(self["mail"]["content"]).change();
                        var inputContent = self["mail"]["content"];
                        var iframeContent = $('#contentSystemMailTemplateSentMailManagementDetailModalInputId_textareaId')[0].contentDocument;
                        iframeContent.open();
