...
var MPSSystemMailMailSentMailManagementAddSystemMailMail = function () {
    // Shared variables
    const form = document.getElementById('SystemMailMailSentMailManagementCreateId_form');

    // Init ck editor
    const initCKEditor = () => {
        // Define all elements for ck editor
        const elements = [
            '#contentSystemMailMailSentMailManagementCreateInputId', 
        ];
    ...
    var initAddSystemMailMail = () => {
        ...
        const submitButton = form.querySelector('[systemmailmailsentmailmanagement-add-action="submit"]');
            ...
            if (validator) {
                validator.validate().then(function (status) {
                    ...
                    if (status == 'Valid') {
                        ...
                        obj.tCreateNewPostFormApi('SystemMailMailSentMailManagementCreateId_form');
...
//
        ...
        const cancelButton = form.querySelector('[systemmailmailsentmailmanagement-add-action="cancel"]');
        cancelButton.addEventListener('click', e => {
            e.preventDefault();

            Swal.fire({
                html: "Bạn có chắc muốn hủy quá trình thêm mới <b>Mail cần gửi</b> này?",
                icon: "warning",
