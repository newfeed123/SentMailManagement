...
var MPSSystemMailMailSentMailManagementEditSystemMailMail = function () {
    // Shared variables
    const form = document.getElementById('SystemMailMailSentMailManagementEditId_form');
    
    // Init ck editor
    const initCKEditor = () => {
        // Define all elements for ck editor
        const elements = [
            '#contentSystemMailMailSentMailManagementEditInputId', 
        ];
    ...
    var initEditSystemMailMail = () => {
        ...
        if(obj_uuid != null && obj_uuid != undefined && obj_uuid != ""){
            ...
            obj.callAjax.then(function(data) {
                new SystemMailMailSentMailManagement(data).tFillEditForm('SystemMailMailSentMailManagementEditId_form');
            })
        }
        // Submit button handler
        const submitButton = form.querySelector('[systemmailmailsentmailmanagement-edit-action="submit"]');
            ...
            if (validator) {
                validator.validate().then(function (status) {
                    ...
                    if (status == 'Valid') {
                        ...
                        obj.tUpdatePostApi('SystemMailMailSentMailManagementEditId_form');
                            ...
                            if (data.uuid != undefined && data.uuid != null && data.uuid != ""){
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        $(location).prop('href', `${org_app_prefixId}/SentMailManagement/SystemMailMail/detail/${data.uuid}/`);
                                    }
                                });
                            }
        ...
        const cancelButton = form.querySelector('[systemmailmailsentmailmanagement-edit-action="cancel"]');
