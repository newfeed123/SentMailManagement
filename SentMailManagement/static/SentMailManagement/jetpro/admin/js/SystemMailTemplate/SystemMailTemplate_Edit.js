...
var MPSSystemMailTemplateSentMailManagementEditSystemMailTemplate = function () {
    // Shared variables
    const form = document.getElementById('SystemMailTemplateSentMailManagementEditId_form');
    
    // Init ck editor
    const initCKEditor = () => {
        // Define all elements for ck editor
        const elements = [
            '#contentSystemMailTemplateSentMailManagementEditInputId', 
        ];
    ...
    var initEditSystemMailTemplate = () => {
        ...
        if(obj_uuid != null && obj_uuid != undefined && obj_uuid != ""){
            ...
            obj.callAjax.then(function(data) {
                new SystemMailTemplateSentMailManagement(data).tFillEditForm('SystemMailTemplateSentMailManagementEditId_form');
            })
        }
        // Submit button handler
        const submitButton = form.querySelector('[systemmailtemplatesentmailmanagement-edit-action="submit"]');
            ...
            if (validator) {
                validator.validate().then(function (status) {
                    ...
                    if (status == 'Valid') {
                        ...
                        obj.tUpdatePostApi('SystemMailTemplateSentMailManagementEditId_form');
                            ...
                            if (data.uuid != undefined && data.uuid != null && data.uuid != ""){
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        $(location).prop('href', `${org_app_prefixId}/SentMailManagement/SystemMailTemplate/detail/${data.uuid}/`);
                                    }
                                });
                            }
        ...
        const cancelButton = form.querySelector('[systemmailtemplatesentmailmanagement-edit-action="cancel"]');
