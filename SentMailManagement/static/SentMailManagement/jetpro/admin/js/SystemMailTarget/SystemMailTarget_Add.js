...
var MPSSystemMailTargetSentMailManagementAddSystemMailTarget = function () {
    // Shared variables
    const form = document.getElementById('SystemMailTargetSentMailManagementCreateId_form');

    // Init ck editor
    const initCKEditor = () => {
        // Define all elements for ck editor
        const elements = [
            '#contentSystemMailTargetSentMailManagementCreateInputId', 
        ];
    ...
    var initAddSystemMailTarget = () => {
        ...
        var validator = FormValidation.formValidation(
            ...
            'email_template': {
                validators: {
                    notEmpty: {
                        message: 'Mẫu mail không được để trống'
        ...
        const submitButton = form.querySelector('[systemmailtargetsentmailmanagement-add-action="submit"]');
            ...
            if (validator) {
                validator.validate().then(function (status) {
                    ...
                    if (status == 'Valid') {
                        ...
                        obj.tCreateNewPostFormApi('SystemMailTargetSentMailManagementCreateId_form');
                            ...
                            if (data.uuid != undefined && data.uuid != null && data.uuid != ""){
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        $(location).prop('href', `${org_app_prefixId}/SentMailManagement/SystemMailTarget/detail/${data.uuid}/`);
                                    }
                                });
                            }
...
//
        ...
        const cancelButton = form.querySelector('[systemmailtargetsentmailmanagement-add-action="cancel"]');
        cancelButton.addEventListener('click', e => {
            e.preventDefault();

            Swal.fire({
                html: "Bạn có chắc muốn hủy quá trình thêm mới <b>Cá nhân/nhóm nhận mail</b> này?",
                icon: "warning",
