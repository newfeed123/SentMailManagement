...
class SystemMailMailSentMailManagement {
    ...
    constructor(data = null) {
        if (data != null) {
            ...
            if (data.hasOwnProperty('list_mail_target')) {
                this.list_mail_target = data.list_mail_target;
            }
            if (data.hasOwnProperty('file_attachment')) {
                this.file_attachment = data.file_attachment;
            }
    ...
    tGetFormData(formId = null) {
        ...
        if (formEle.length > 0) {
            ...
            var chEle_mail_to = formEle.find("#mail_toSystemMailMailSentMailManagementInputId");
            if (chEle_content.length > 0) {
                this.mail_to = chEle_mail_to.val();
            } 
                ...
                var j_ele_mail_sender = $("#mail_senderSystemMailMailSentMailManagementInputId");
                    if (j_ele_mail_sender.length > 0){
                        ...
                        if(mail_sender_select_value == undefined || mail_sender_select_value == null){
                            ...
                            j_ele_mail_sender.append(new Option(mail_sender_name, mail_sender_value,true,true));
                        }
                    }
            }
            catch(err) {
                ...
                var j_ele_mail_target = $("#mail_targetSystemMailMailSentMailManagementInputId");
                    if (j_ele_mail_target.length > 0){
                        ...
                        if(mail_target_select_value == undefined || mail_target_select_value == null){
                            ...
                            j_ele_mail_target.append(new Option(mail_target_name, mail_target_value,true,true));
                        }
                    }
            }
            catch(err) {
                ...
                var j_ele_addressee = $("#addresseeSystemMailMailSentMailManagementInputId");
                    if (j_ele_addressee.length > 0){
                        ...
                        if(addressee_select_value == undefined || addressee_select_value == null){
                            ...
                            j_ele_addressee.append(new Option(addressee_name, addressee_value,true,true));
                        }
                    }
            }
            catch(err) {
    ...
    tFillEditModalForm(formId = null) {
        self = this
            ...
            try{
                ...
                if (j_ele_content.length > 0){
                    if (j_ele_content.attr('name') != 'uuid'){
                        if (MPSSystemMailMailSentMailManagementEditSystemMailMail_ckeditor_list.hasOwnProperty('#contentSystemMailMailSentMailManagementEditModalInputId')){
                            MPSSystemMailMailSentMailManagementEditSystemMailMail_ckeditor_list["#contentSystemMailMailSentMailManagementEditModalInputId"].setData(self.content);
                        }
                    }
                }
                else{
            ...
            try{
                var j_ele_mail_sender = $("#mail_senderSystemMailMailSentMailManagementDetailInputId");
                    if (j_ele_mail_sender.length > 0){
                        ...
                        var mail_sender_value = self["mail_sender"]["uuid"];
                        var mail_sender_name = `<a href="${org_app_prefix}/SentMailManagement/SystemMailSenderConfig/detail/${mail_sender_value}/" class="text-gray-600 text-hover-primary me-2">${self["mail_sender"]["name"]}</a>`
            ...
            try{
                var j_ele_mail_target = $("#mail_targetSystemMailMailSentMailManagementDetailInputId");
                    if (j_ele_mail_target.length > 0){
                        ...
                        var mail_target_value = self["mail_target"]["uuid"];
                        var mail_target_name = `<a href="${org_app_prefix}/SentMailManagement/SystemMailTarget/detail/${mail_target_value}/" class="text-gray-600 text-hover-primary me-2">${self["mail_target"]["name"]}</a>`
            ...
            try{
                var j_ele_addressee = $("#addresseeSystemMailMailSentMailManagementDetailInputId");
                    if (j_ele_addressee.length > 0){
                        ...
                        var addressee_value = self["addressee"]["uuid"];
                        var addressee_name = `<a href="${org_app_prefix}/HRAccountManagement/HRAccount/detail/${addressee_value}/" class="text-gray-600 text-hover-primary me-2">${self["addressee"]["name"]}</a>`
            ...
            try{
                ...
                if (j_ele_content.length > 0){
                    if (j_ele_content.attr('name') != 'uuid'){
                        ...
                        var iframeContent = $('#contentSystemMailMailSentMailManagementDetailModalInputId')[0].contentDocument;
                        iframeContent.open();
    ...
    tCreateNewPostFormApi(formId = null) {
        ...
        var arr_table = [];
        if (formId == null) {
            formId = 'ComposeEmailSentMailManagementCreateId_form';
        }
        ...
        if (!is_save_self_table) {
            this.callAjax = $.ajax({
                url: SYSTEMMAILMAILSENTMAILMANAGEMENT_CREATE_URL_COMPOSE_EMAIL,
                type: "POST",
                async: false,
                cache: false,
                timeout: 30000,
                ...
                if (this.hasOwnProperty(attr)) {
                    ...
                    if(attr=="content"){
                        ...
                        if(this[attr] != undefined && this[attr] !== "None"){
                            ...
                            $('body').append(newDiv);
                            newDiv.html(this[attr])
                            var decodedHTML = newDiv.text();
                            newDiv.remove()
                            // $("#displaySystemMailTargetSentMailManagementCreateModalInputId_textareaId").html(decodedHTML);
                    
                            row_data[col_num] = decodedHTML;
                        }
                        else{
                            row_data[col_num] = '';
