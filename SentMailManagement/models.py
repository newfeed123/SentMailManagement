...
class SystemMailTemplate(models.Model):
    ...
    # (subject_of_template) Tiêu đề của mẫu mail gửi đi :
    subject_of_template = models.CharField(
                                editable=True,
                                blank=False,
                                null=False,
                                ...
                                default="",
                                max_length=2000,
                                help_text=_('Tiêu đề của mẫu mail'),
                            )
...
class SystemMailMail(models.Model):
    ...
    def set_email_template_values(self,fk):
        try:
            if fk != "" and fk != None:
                crr_pk = SystemMailTemplate.objects.filter(pk=fk).values("uuid","code","friendly_code","name", "content", "subject_of_template").first()
                if crr_pk is not None:
                    self.email_template = crr_pk
                    print("set or update email_template success!!")  
                else:
    ...
    def send_mail_sys(self, template_code, mail_to, lst_value=None, crr_mail_sender=None, crr_mail_target=None):
        ...
        try:
            ...
            # nếu ko có, sẽ tự động sử dụng using('default') : mail_template = SystemMailTemplate.objects.using('default').filter(friendly_code=template_code).first()
            ...
            mail_template_dict = {
                ...
                'subject_of_template': mail_template.subject_of_template,
            }
            ...
            if crr_mail_sender is not None:
                ...
                if mail_sender is None:
                    mail_sender = SystemMailSenderConfig.objects.filter(active=True).values("uuid","friendly_code","code","name").first()
            ...
            if 'fullname' in lst_value:
                # Nếu 'fullname' tồn tại, thực hiện thay thế
                subject = mail_template.subject_of_template.replace("$fullname", lst_value["fullname"])
            if 'crr_organization' in lst_value:
                # Nếu 'crr_organization' tồn tại, thực hiện thay thế
                subject = subject.replace("$crr_organization", lst_value["crr_organization"])
            try:
    ...
    def send_mail_front_end_to_many_people(self, uuid_mail_target, crr_mail_sender=None, content=None):
        try:
            ...
            try:
                ...
                # self.content = content
                self.content = self.email_template["content"]
                self.mail_sender = mail_sender
                self.status_text = "Khởi tạo"
                super().save()
    ...
    def auto_compose_email_sys_to_many_people(self):
        try:
            ...
            # Từ email gửi đi lấy ra cấu hình mail server
            # mail_server = SystemMailServerConfig.objects.filter(uuid=mail_sender.email_server["uuid"]).first()
            mail_server = SystemMailServerConfig.objects.filter(is_default=True).first()
            ...
            if mail_server is None:
                ...
                if mail_sender is None:
                    ...
                    try:
                        ...
                        attachment = self.file_attachment  # Lấy trường FileField từ đối tượng mô hình      
                        if attachment:  # Kiểm tra xem trường FileField có giá trị hay không
                            part = MIMEBase("application", "octet-stream")
                            with open(attachment.path, "rb") as file:  # Mở tệp đính kèm
                        ...
                        mail_template = self.content
                        for item in self.list_mail_target:
                            DISPLAY_NAME_EMAIL = 'HRM360' #tên hiển thị cho người gửi.
                            email_message = MIMEMultipart() #Tạo một email_message MIME multipart
                            email_message['From'] = formataddr((DISPLAY_NAME_EMAIL, sender_email)) 
                            email_message['To'] = item["email"]
                            email_message['Subject'] = self.name
                            self.content = mail_template.replace("$user_fullname",item["name"]).replace("$hrm_domain",DOMAIN_NAME)
                            email_message.attach(MIMEText(self.content, "html")) #Đính kèm nội dung HTML vào email_message.
                            if part:  # Nếu có attachment, thì mới đính kèm vào email_message
                                email_message.attach(part) #Đính kèm nội dung HTML vào email_message.
                            # email_string = email_message.as_string() #Chuyển đổi email_message email thành chuỗi.
                            context = ssl.create_default_context() 
                            ...
                            with smtplib.SMTP_SSL(smtp_server_host, port, context=context) as server:
                                server.ehlo() #Xác định máy khách đến máy chủ.  
                                server.login(sender_email, password) #Đăng nhập vào máy chủ email.
                                server.sendmail(sender_email, item["email"], email_message.as_string()) #Gửi email.
                                self.status_text = "Successful" 
                                self.status_code = "200"
                            ...
                            email_has_sent.to_email = item["email"]
    ...
    def auto_send_email_sys_to_many_people(self):
        try:
            ...
            for item in field_target:
                # Kiểm tra xem đối tượng có khóa 'email' và 'name' không
                email = item.get('email')
                name = item.get('name')
                if email is not None and name is not None:
                    # Nếu cả hai khóa đều tồn tại, thêm một từ điển mới vào danh sách
                    email_list.append({'email': email, 'name': name})
            print('mail_server =', mail_server)
             # Nếu không tìm thấy SMTP server, đặt thông báo lỗi và trả về
            if mail_server is None:
                self.status_text = "[MPS] - Error Not Found SMTP Server Config"
                ...
                if mail_sender is None:
                    ...
                    try:
                        ...
                        subject_of_email_message = self.email_template["subject_of_template"]
                        # for receiver_email_item in receiver_email:
                        for receiver_item in email_list:
                            DISPLAY_NAME_EMAIL = 'HRM360' #tên hiển thị cho người gửi.
                            email_message = MIMEMultipart() #Tạo một email_message MIME multipart
                            email_message['From'] = formataddr((DISPLAY_NAME_EMAIL, sender_email)) 
                            # email_message['To'] = receiver_email_item
                            email_message['To'] = receiver_item["name"]
                            # email_message['Subject'] = self.name
                            temp_subject = subject_of_email_message #biến tạm thời
                            email_message['Subject'] = temp_subject.replace("$fullname", receiver_item["name"])
                            self.content = self.email_template["content"]
                            # content_replace = self.content.replace("$username", receiver_email_item).replace("$hrm_domain", DOMAIN_NAME).replace("$password", "HRM360@MPHR")
                            content_replace = self.content.replace("$username", receiver_item["email"]).replace("$hrm_domain", DOMAIN_NAME).replace("$password", "HRM360@MPHR")
                            email_message.attach(MIMEText(content_replace, "html")) #Đính kèm nội dung HTML vào email_message.
                            email_string = email_message.as_string() #Chuyển đổi email_message email thành chuỗi.
                            context = ssl.create_default_context() 
                            ...
                            with smtplib.SMTP_SSL(smtp_server_host, port, context=context) as server:
                                server.ehlo() #Xác định máy khách đến máy chủ.  
                                server.login(sender_email, password) #Đăng nhập vào máy chủ email.
                                server.sendmail(sender_email, receiver_item["email"], email_string) #Gửi email.
                                self.status_text = "Successful" 
                                self.status_code = "200"
                                mail_sender.number_sent = mail_sender.number_sent + 1
                                mail_sender.save()
                        ...
                        emails = [item['email'] for item in email_list]
                        to_email_string = ", ".join(emails)
